...
internal unsafe interface ICorProfilerInfo4 : ICorProfilerInfo3
{
     ...
     * re-JITs.
     */
    HResult RequestReJIT(uint cFunctions, ModuleId* moduleIds, MdMethodDef* methodIds);
     ...
     *
     */
    HResult RequestRevert(uint cFunctions, ModuleId* moduleIds, MdMethodDef* methodIds, HResult* status);
     ...
     */
    HResult GetCodeInfo3(FunctionId functionID, ReJITId reJitId, uint cCodeInfos, out uint pcCodeInfos, COR_PRF_CODE_INFO* codeInfos);

    /*
     * Same as GetFunctionFromIP, but also returns which re-JITed version is
     * associated with the IP address.
     */
    HResult GetFunctionFromIP2(nint ip, out FunctionId functionId, out ReJITId reJitId);

    /*
     * GetReJITIDs can be used to find all of the re-JITed versions of the
     * given function.
     */
    HResult GetReJITIDs(FunctionId functionId, uint cReJitIds, out uint pcReJitIds, ReJITId* reJitIds);

    /*
     * Same as GetILToNativeMapping, but allows the code profiler to specify
     * which re-JITed version it applies to.
     */
    HResult GetILToNativeMapping2(FunctionId functionId, ReJITId reJitId, uint cMap, out uint pcMap, COR_DEBUG_IL_TO_NATIVE_MAP* map);
     ...
     */
    HResult GetObjectSize2(ObjectId objectId, out nint pcSize);
