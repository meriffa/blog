...
internal unsafe interface ICorProfilerInfo : IUnknown
{
     ...
     */
    HResult GetClassFromToken(ModuleId ModuleId, MdTypeDef typeDef, out ClassId pClassId);
     ...
     * in all cases.
     */
    HResult GetCodeInfo(FunctionId FunctionId, out byte* pStart, out uint pcSize);
    /*
     ...
     */
    HResult GetFunctionFromToken(ModuleId ModuleId, MdToken token, out FunctionId pFunctionId);
     ...
     * before using it.
     */
    HResult GetHandleFromThread(ThreadId ThreadId, out nint phThread);
     ...
     */
    HResult GetObjectSize(ObjectId ObjectId, out uint pcSize);
     ...
     */
    HResult IsArrayClass(ClassId ClassId, out CorElementType pBaseElemType, out ClassId pBaseClassId, out uint pcRank);

    /*
     * The code profiler calls GetThreadInfo to obtain the current Win32 thread ID for
     * the specified thread.
     */
    HResult GetThreadInfo(ThreadId ThreadId, out uint pdwWin32ThreadId);
     ...
     */
    HResult GetClassIdInfo(ClassId ClassId, out ModuleId pModuleId, out MdTypeDef pTypeDefToken);
     ...
     */
    HResult GetFunctionInfo(FunctionId FunctionId, out ClassId pClassId, out ModuleId pModuleId, out MdToken pToken);
     ...
     */
    HResult SetEnterLeaveFunctionHooks(void* pFuncEnter, void* pFuncLeave, void* pFuncTailcall);
     ...
     */
    HResult GetTokenAndMetaDataFromFunction(FunctionId FunctionId, out Guid riid, out void* ppImport, out MdToken pToken);
     ...
     */
    HResult GetModuleInfo(ModuleId ModuleId, out nint ppBaseLoadAddress, uint cchName, out uint pcchName, char* szName, out AssemblyId pAssemblyId);
     ...
     */
    HResult GetModuleMetaData(ModuleId ModuleId, CorOpenFlags dwOpenFlags, Guid* riid, out nint ppOut);
     ...
     * P/Invoke method).
     */
    HResult GetILFunctionBody(ModuleId ModuleId, MdMethodDef methodId, out byte* ppMethodHeader, out uint pcbMethodSize);
     ...
     */
    HResult GetILFunctionBodyAllocator(ModuleId ModuleId, out void* ppMalloc);
     ...
     */
    HResult SetILFunctionBody(ModuleId ModuleId, MdMethodDef methodid, nint newILMethodHeader);

    /*
     * Retrieve app domain information given its id.
     */
    HResult GetAppDomainInfo(AppDomainId appDomainId, uint cchName, out uint pcchName, char* szName, out ProcessId pProcessId);

    /*
     * Retrieve information about an assembly given its ID.
     */
    HResult GetAssemblyInfo(AssemblyId assemblyId, uint cchName, out uint pcchName, char* szName, out AppDomainId pAppDomainId, out ModuleId pModuleId);
     ...
     *
     */
    HResult SetILInstrumentedCodeMap(FunctionId FunctionId, int fStartJit, uint cILMapEntries, CorIlMap* rgILMapEntries);

    /*
     * DEPRECATED.
     */
    HResult GetInprocInspectionInterface(out void* ppicd);

    /*
     * DEPRECATED.
     */
    HResult GetInprocInspectionIThisThread(out void* ppicd);
     ...
     */
    HResult GetThreadContext(ThreadId ThreadId, out ContextId pContextId);

    /*
     * DEPRECATED.
     */
    HResult BeginInprocDebugging(int thisThreadOnly, out uint pdwProfilerContext);

    /*
     * DEPRECATED.
     */
    HResult EndInprocDebugging(uint profilerContext);
     ...
     */
    HResult GetILToNativeMapping(FunctionId FunctionId, uint cMap, out uint pcMap, CorDebugIlToNativeMap* map);
